...
function movementItemRow(data = {}) {
  ...
  row.innerHTML = `<div class="entry-item-select-row"><label>${itemLabel} *<select class="movement-item-select no-select-arrow" required></select></label><button type="button" class="remove-movement-item" aria-label="Remover item">×</button></div>${fuel
...
function openContextSelector(type, target) {
  ...
  $("#contextSelectorTitle").textContent =
    ...
    type === "MOTORISTA" ? "Selecionar motorista" :
    type === "FORMA_PAGAMENTO" ? "Selecionar forma de pagamento" : "Selecionar fornecedor";
...
$$(".entry-quick-add").forEach((button) => button.onclick = () => {
  const type = button.dataset.registerType;
  const fieldName = type === "MOTORISTA" ? "motorista" : type === "FORNECEDOR" ? "fornecedor" : "formaPagamento";
  entryContext = { type, target: $(`#entryForm [name=${fieldName}]`) };
  $("#registerGroup").value = type;
  openRegister();
});
