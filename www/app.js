...
function drawLineChart(canvas, labels, values, format = "money", color = "#246b9e", options = {}) {
  const { showYAxisLabels = true, showPointValues = true, yMin, yMax } = options;
  ...
  const { c, w, h } = canvasBase(canvas), left = 58, right = 22, top = 28, bottom = 58,
    ...
    min = Number.isFinite(Number(yMin)) ? Number(yMin) : Math.min(0, dataMin),
    max = Number.isFinite(Number(yMax)) ? Number(yMax) : Math.max(dataMax, min + 1),
...
function odometerChartData(vehicle, allMovements, start, end) {
  ...
  const values = points.map((point) => point.km), max = values.length ? Math.max(...values) : baseline;
  return { labels, values, yMin: baseline || (values.length ? Math.min(...values) : 0), yMax: max + Math.max(1, (max - (baseline || max)) * .08) };
}
...
function renderNewCharts(ms, { vehicle, allMovements, start, end } = {}) {
  ...
  drawLineChart($("#chartOdometer"), odometer.labels, odometer.values, "km", "#246b9e", { showPointValues:false, yMin:odometer.yMin, yMax:odometer.yMax });
...
function renderCharts() {
  ...
  const selected = $("#chartVehicle").value,
    selectedObject = vehicles.find((item) => item.nome === selected) || selectedVehicleObject(),
    allVisible = filterByPeriod(filtered(selected), "chart"),
    validPeriod = periodIsValid("chart");
  updateStaticVehicleSummary("chartVehicleName", "chartVehicleStatus", selectedObject);
  $("#chartPeriodLabel").textContent = "Padrão: últimos 6 meses. Altere as datas para consultar outro período.";
  ...
  renderNewCharts(allVisible, { vehicle, allMovements: filtered(selected), start, end });
}
