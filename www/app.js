...
function periodChartData(ms, start, end) {
  ...
  buckets.forEach((bucket) => byKey.set(bucket.key, { net: 0, distance: 0, liters: 0, movementCount: 0 }));
  ms.forEach((movement) => {
    ...
    if (movement.grupo === "COMBUSTÍVEL" && movement.tanque_completo !== "NAO" && +movement.quantidade_litros > 0 && +movement.distancia_abastecimento_km > 0) {
      item.distance += +movement.distancia_abastecimento_km;
      item.liters += +movement.quantidade_litros;
    }
  });
  return {
    granularity,
...
function odometerReferencePointsByYear(points) {
  const groups = new Map();
  points.forEach((point) => {
    const year = chartDate(point.date).getFullYear();
    if (!groups.has(year)) groups.set(year, []);
    groups.get(year).push(point);
  });
  const selected = [];
  [...groups.entries()].sort((a, b) => a[0] - b[0]).forEach(([year, yearPoints]) => {
    const ordered = [...yearPoints].sort((a, b) => a.date.localeCompare(b.date) || a.km - b.km);
    if (ordered.length <= 2) {
      selected.push(...ordered.map((point) => ({ ...point, year })));
      return;
    }
    const first = ordered[0], last = ordered.at(-1);
    const firstTime = chartDate(first.date).getTime(), lastTime = chartDate(last.date).getTime();
    const middleTime = firstTime + (lastTime - firstTime) / 2;
    const middle = ordered.slice(1, -1).reduce((best, point) => {
      const distance = Math.abs(chartDate(point.date).getTime() - middleTime);
      return !best || distance < best.distance ? { point, distance } : best;
    }, null)?.point;
    [first, middle, last].filter(Boolean).forEach((point) => {
      if (!selected.some((item) => item.date === point.date && item.km === point.km)) selected.push({ ...point, year });
    });
  });
  return selected.sort((a, b) => a.date.localeCompare(b.date) || a.km - b.km);
}
...
function odometerChartData(vehicle, allMovements, start, end) {
  ...
  const longPeriod = chartPeriodGranularity(start, end) === "year";
  const displayPoints = longPeriod ? odometerReferencePointsByYear(normalizedPoints) : normalizedPoints;
  const labelIndexes = new Set();
  if (!longPeriod && displayPoints.length) {
    labelIndexes.add(0);
    labelIndexes.add(Math.floor((displayPoints.length - 1) / 2));
    labelIndexes.add(displayPoints.length - 1);
  }
  ...
  return {
    ...
    labels: displayPoints.map((point, index) => longPeriod
      ? odometerMonthLabel(point.date, false)
      : (labelIndexes.has(index) ? odometerMonthLabel(point.date, true) : "")),
    years: displayPoints.map((point) => chartDate(point.date).getFullYear()),
    values,
    yMin,
    yMax,
  };
...
function drawOdometerChart(canvas, data) {
  const minHeight = data.longPeriod ? 350 : 310;
  const { c, w, h } = canvasBase(canvas, minHeight), right = 24, top = 28,
    bottom = data.longPeriod ? 82 : 58,
  ...
  points.forEach((point, index) => {
    ...
    if (!label) return;
    c.textAlign = point.x < left + 18 ? "left" : point.x > w - right - 18 ? "right" : "center";
    c.fillText(label, point.x, h - bottom + 20);
  });
  if (data.longPeriod) {
    const yearGroups = [];
    data.years.forEach((year, index) => {
      let group = yearGroups.find((item) => item.year === year);
      if (!group) { group = { year, indexes: [] }; yearGroups.push(group); }
      group.indexes.push(index);
    });
    c.font = "700 11px system-ui";
    yearGroups.forEach((group) => {
      const first = points[group.indexes[0]], last = points[group.indexes.at(-1)];
      if (!first || !last) return;
      const center = (first.x + last.x) / 2;
      const lineStart = group.indexes.length === 1 ? Math.max(left, center - 18) : first.x;
      const lineEnd = group.indexes.length === 1 ? Math.min(w - right, center + 18) : last.x;
      c.strokeStyle = "#6f8ba199"; c.lineWidth = 1;
      c.beginPath(); c.moveTo(lineStart, h - bottom + 34); c.lineTo(lineEnd, h - bottom + 34); c.stroke();
      c.fillStyle = chartTextColor(); c.textAlign = "center"; c.fillText(String(group.year), center, h - bottom + 52);
    });
  }
  if (!points.length) {
    c.fillStyle = chartTextColor(); c.font = "700 13px system-ui"; c.textAlign = "center";
...
function renderNewCharts(ms, { vehicle, allMovements, start, end } = {}) {
  ...
  if ($("#chartConsumptionTitle")) $("#chartConsumptionTitle").textContent = monthly
    ...
    : "Consumo médio anual de combustível (km/L)";
  if ($("#chartConsumptionNote")) $("#chartConsumptionNote").textContent = "Distância válida do período ÷ litros abastecidos. Meses sem dados não são tratados como consumo zero.";
  ...
  drawLineChart($("#chartAnnualConsumption"), timeline.labels, timeline.consumption, "consumption", "#246b9e");
