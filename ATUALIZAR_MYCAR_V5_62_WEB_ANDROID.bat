...
chcp 65001 >nul
title Atualizacao MyCar+ V5.62 - Web e Android

rem ============================================================
rem CONFIGURACAO
rem ============================================================
set "VERSAO=5.62"
set "ZIP=%USERPROFILE%\Downloads\MYCAR_PLUS_V5_62_MASTER.zip"
set "PROJETO=%USERPROFILE%\Documents\GitHub\MyCarPlus"
set "TEMP=%USERPROFILE%\Downloads\MYCAR_PLUS_V5_62_TEMP"
set "LOG=%USERPROFILE%\Downloads\ATUALIZACAO_MYCAR_V5_62_LOG.txt"

for /f "delims=" %%I in ('powershell.exe -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set "STAMP=%%I"
set "BACKUP=%USERPROFILE%\Documents\GitHub\MyCarPlus_BACKUP_ANTES_V5_62_%STAMP%"
...
if not exist "%ZIP%" (
    set "ZIP_ENCONTRADO="
    for /f "delims=" %%Z in ('dir /B /A:-D /O:-D "%USERPROFILE%\Downloads\MYCAR_PLUS_V5_62_MASTER*.zip" 2^>nul') do (
...
set "FONTE=%TEMP%\MYCAR_PLUS_V5_62_MASTER"
...
robocopy "!FONTE!" "%PROJETO%" /E /COPY:DAT /DCOPY:DAT /R:2 /W:2 ^
 ...
 /XF ATUALIZACAO_MYCAR_V5_62_LOG.txt >>"%LOG%" 2>&1
set "RC=!ERRORLEVEL!"
if !RC! GEQ 8 (
...
rem Remove instaladores parciais antigos da V5.62 que dependiam de PS1.
for %%P in (
    "%PROJETO%\APLICAR_ATUALIZACAO_MYCAR_V5_62.ps1"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_62.bat"
) do (
    if exist "%%~fP" (
...
git commit -m "Atualiza MyCar+ para V5.62" >>"%LOG%" 2>&1
if errorlevel 1 (
