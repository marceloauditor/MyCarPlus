...
chcp 65001 >nul
title Atualizacao MyCar+ V5.70 - Web e Android

rem ============================================================
rem CONFIGURACAO
rem ============================================================
set "VERSAO=5.70"
set "ZIP=%USERPROFILE%\Downloads\MYCAR_PLUS_V5_70_TREE.zip"
set "PROJETO=%USERPROFILE%\Documents\GitHub\MyCarPlus"
set "WORKTEMP=%USERPROFILE%\Downloads\MYCAR_PLUS_V5_70_TREE_TEMP"
set "LOG=%USERPROFILE%\Downloads\ATUALIZACAO_MYCAR_V5_70_LOG.txt"

for /f "delims=" %%I in ('powershell.exe -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set "STAMP=%%I"
set "BACKUP=%USERPROFILE%\Documents\GitHub\MyCarPlus_BACKUP_ANTES_V5_70_%STAMP%"
...
if not exist "%ZIP%" (
    set "ZIP_ENCONTRADO="
    for /f "delims=" %%Z in ('dir /B /A:-D /O:-D "%USERPROFILE%\Downloads\MYCAR_PLUS_V5_70_TREE*.zip" 2^>nul') do (
...
rem ============================================================
rem LIMPEZA DE RESIDUOS DE ATUALIZACOES ANTERIORES
rem Remove somente pastas indevidas conhecidas que podem gerar
rem classes duplicadas no Android. O backup acima preserva tudo.
rem ============================================================
>>"%LOG%" echo Limpando residuos Android de atualizacoes anteriores...

for %%D in (
    "%PROJETO%\android\app\src\main\java\br\com\marceloauditor\mycarplus\lx\MYCAR_PLUS_V5_70_TEMP"
    "%PROJETO%\android\app\src\main\java\br\com\marceloauditor\mycarplus\lx\MYCAR_PLUS_V5_70_MASTER"
    "%PROJETO%\android\app\src\main\java\br\com\marceloauditor\mycarplus\lx\MYCAR_PLUS_V5_70_TREE"
) do (
    if exist "%%~fD" (
        >>"%LOG%" echo Excluindo pasta indevida: %%~fD
        rmdir /S /Q "%%~fD" >>"%LOG%" 2>&1
    )
)

rem Remove os testes padrao antigos do Capacitor para evitar duplicate class.
for %%D in (
    "%PROJETO%\android\app\src\androidTest\java\com\getcapacitor\myapp"
    "%PROJETO%\android\app\src\test\java\com\getcapacitor\myapp"
) do (
    if exist "%%~fD" (
        >>"%LOG%" echo Excluindo teste antigo: %%~fD
        rmdir /S /Q "%%~fD" >>"%LOG%" 2>&1
    )
)
...
if exist "%WORKTEMP%" rmdir /S /Q "%WORKTEMP%" >>"%LOG%" 2>&1
mkdir "%WORKTEMP%" >>"%LOG%" 2>&1
...
set "FONTE=%WORKTEMP%\MYCAR_PLUS_V5_70_TREE"
if not exist "!FONTE!\package.json" set "FONTE="

if not defined FONTE (
    for /f "delims=" %%F in ('dir /B /S /A:-D "%WORKTEMP%\package.json" 2^>nul') do (
...
robocopy "!FONTE!" "%PROJETO%" /E /COPY:DAT /DCOPY:DAT /R:2 /W:2 ^
 ...
 /XF ATUALIZACAO_MYCAR_V5_70_LOG.txt >>"%LOG%" 2>&1
set "RC=!ERRORLEVEL!"
if !RC! GEQ 8 (
...
for %%B in (
    ...
    "%PROJETO%\ATUALIZAR_MYCAR_V5_55_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_56_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_57_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_58_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_59_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_60_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_61_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_62_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_63_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_64_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_65_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_66_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_67_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_68_WEB_ANDROID*.bat"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_69_WEB_ANDROID*.bat"
) do (
    if exist "%%~fB" (
...
rem Remove instaladores parciais antigos da V5.70 que dependiam de PS1.
for %%P in (
    "%PROJETO%\APLICAR_ATUALIZACAO_MYCAR_V5_70.ps1"
    "%PROJETO%\ATUALIZAR_MYCAR_V5_70.bat"
) do (
    if exist "%%~fP" (
...
git commit -m "Atualiza MyCar+ para V5.70" >>"%LOG%" 2>&1
if errorlevel 1 (
