# MyCar+ V5.50 — Pacote-fonte oficial
...
Na pasta Downloads, mantenha o arquivo `MYCAR_PLUS_V5_50_MASTER.zip` e execute:

```powershell
.\ATUALIZAR_MYCAR_V5_50_WEB_ANDROID.bat
```

O BAT cria backup, instala dependências, sincroniza Web e Android, valida a coesão e gera o APK de teste.

Se o navegador salvar o ZIP com sufixo como `(1)`, o BAT V5.50 localiza automaticamente o arquivo mais recente compatível na pasta Downloads.
...
A V5.50 padroniza os alertas no grupo **MANUTENÇÃO**, mantém o vínculo obrigatório com o veículo selecionado e impede avisos ativos para veículos inativos. A exclusão remove somente o alerta e sua programação futura; os registros do histórico técnico e os movimentos de manutenção são preservados.

O BAT da V5.50 é autocontido e não depende de arquivo `.ps1`.
