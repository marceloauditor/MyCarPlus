...
window.MyCarReportManager?.registerCoverFactory(createReportCoverFile);
...
async function shareReportHtmlFromViewer(data = {}) {
  ...
  const coverFile = await createReportCoverFile(jobName);
  const coverBase64 = coverFile ? String(await new Promise((resolve) => {
    const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || "").split(",")[1] || ""); reader.readAsDataURL(coverFile);
  })) : "";
  if (isNativeApp()) {
    if (bridge && typeof bridge.shareHtmlWithCover === "function" && coverFile && coverBase64) {
      try { bridge.shareHtmlWithCover(jobName, html, coverFile.name, coverBase64); return; }
      catch (error) { console.error("Falha no compartilhamento múltiplo Android:", error); }
    }
    if (bridge && typeof bridge.shareHtml === "function") {
      try { bridge.shareHtml(jobName, html); return; }
      catch (error) { console.error("Falha ao compartilhar o relatório pelo Android:", error); }
    }
    alert("O módulo de compartilhamento não está disponível. Feche e abra novamente o aplicativo."); return;
  }
  const safeName = jobName.replace(/[^a-zA-Z0-9._-]+/g, "_").replace(/_+/g, "_") || "MyCarPlus_Relatorio";
  const htmlFile = new File([html], `${safeName}.html`, { type: "text/html;charset=utf-8" });
  const files = coverFile ? [coverFile, htmlFile] : [htmlFile];
  try {
    if (navigator.share && navigator.canShare?.({ files })) {
      await navigator.share({ title: jobName.replaceAll("_", " "), files }); return;
    }
  } catch (error) { if (error?.name === "AbortError") return; console.error("Falha no compartilhamento Web:", error); }
...
function exportPdfReport() {
  ...
  function printHtml(){var html=printableDocument();try{if(window.parent&&window.parent.MyCarReportManager){window.parent.MyCarReportManager.print('RELATORIO_EXECUTIVO_MYCAR_PLUS',html);return}if(window.top&&window.top.MyCarReportManager){window.top.MyCarReportManager.print('RELATORIO_EXECUTIVO_MYCAR_PLUS',html);return}if(window.parent&&window.parent!==window){window.parent.postMessage({type:'mycar-print-report-html',jobName:'RELATORIO_EXECUTIVO_MYCAR_PLUS',html:html},'*');return}}catch(e){console.error('Falha ao acionar gerenciador de impressão:',e)}window.print()}function shareHtml(){
    ...
    try{
      ...
      a.href=url;a.download='RELATORIO_EXECUTIVO_MYCAR_PLUS.html';
  ...
  function closeReport(){
    try{
      if(window.parent&&window.parent.MyCarReportManager){window.parent.MyCarReportManager.close();return;}
      if(window.parent&&window.parent!==window){
  ...
  document.getElementById('reportPrintButton').addEventListener('click',printHtml);document.getElementById('reportShareButton').addEventListener('click',shareHtml);
