...
function periodChartData(ms, start, end) {
  ...
  buckets.forEach((bucket) => byKey.set(bucket.key, { net: 0, distance: 0, liters: 0 }));
  ms.forEach((movement) => {
  ...
  return {
    ...
    daily: buckets.map((bucket) => (byKey.get(bucket.key)?.net || 0) / bucket.days),
  };
}
...
function renderCharts() {
  ...
  const { start, end } = periodValues("chart"), vehicle = vehicles.find((item) => item.nome === selected) || selectedVehicleObject();
