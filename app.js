...
function effectivePeriodMessage(info) {
  ...
  return info.wasAdjusted
    ? `Período ajustado aos registros disponíveis: ${formatPeriodDate(info.effectiveStart)} a ${formatPeriodDate(info.effectiveEnd)}.`
...
function odometerChartData(vehicle, allMovements, start, end) {
  ...
  const periodDays = Math.max(1, Math.round((chartDate(end) - chartDate(start)) / 86400000) + 1);
...
function costCompositionData(ms) {
  const s = stats(ms), g = groupTotals(ms), labels = ["Combustível", "Administrativo", "Manutenção"],
    totals = [g.Combustível, g.Administrativo, g.Manutenção], totalCosts = totals.reduce((sum, value) => sum + (+value || 0), 0),
  ...
  return {
    labels, totals,
    perKmLabels: [...labels, "Total dos custos"],
    perKm: [...perKmGroups, s.km ? totalCosts / s.km : 0],
    perKmColors: ["#246b9e", "#246b9e", "#246b9e", "#f28b0c"],
    perDay: totals.map(v=>s.days ? v/s.days : 0),
...
function renderNewCharts(ms, { vehicle, allMovements, start, end } = {}) {
  ...
  drawLineChart($("#chartOdometer"), odometer.labels, odometer.values, "km", "#246b9e", { showPointValues:false, yMin:odometer.yMin, yMax:odometer.yMax, niceAxis:true });
...
async function createReportCoverFile(jobName) {
  ...
  if (!isManual) { ctx.fillText(vehicleName, 600, y+95); ctx.fillText(`${periodText(isAi ? "report" : "report")}`, 600, y+145); }
