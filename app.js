...
window.addEventListener("message", (event) => {
  ...
  if (data.type === "mycar-print-report-html") {
    const html = String(data.html || "");
    const jobName = String(data.jobName || "MyCarPlus_Relatorio");
    if (!html.trim()) return;
    const bridge = window.MyCarNative;
    if (bridge && typeof bridge.printHtml === "function") {
      try { bridge.printHtml(jobName, html); return; }
      catch (error) { console.error("Falha na impressão nativa:", error); }
    }
    const win = window.open("", "_blank");
    if (!win) return alert("Permita janelas pop-up para imprimir o relatório.");
    win.document.open(); win.document.write(html); win.document.close();
    win.addEventListener("load", () => setTimeout(() => win.print(), 150), { once: true });
  }
});
...
function exportPdfReport() {
  ...
  function printHtml(){var html=printableDocument();try{var bridge=window.MyCarNative||(window.parent&&window.parent.MyCarNative)||(window.top&&window.top.MyCarNative);if(bridge&&typeof bridge.printHtml==='function'){bridge.printHtml('RELATORIO_EXECUTIVO_MYCAR_PLUS',html);return}if(window.parent&&window.parent!==window){window.parent.postMessage({type:'mycar-print-report-html',jobName:'RELATORIO_EXECUTIVO_MYCAR_PLUS',html:html},'*');return}}catch(e){console.error('Falha ao acionar impressão nativa:',e)}window.print()}function shareHtml(){
    try{
      if(window.parent&&window.parent!==window){
        window.parent.postMessage({type:'mycar-share-report-html',jobName:'RELATORIO_EXECUTIVO_MYCAR_PLUS',html:printableDocument()},'*');
        return;
      }
    }catch(e){}
    try{
